html += '<div style="font-size: 13px; color: ' + availabilityColor + '; font-weight: 600; margin: 8px 0;">📦 ' + availabilityText + '</div>';
                
                if (product.rating) {{
                    html += '<div style="font-size: 13px; color: #666; margin: 8px 0;">⭐ ' + product.rating + ' estrellas (' + product.reviews + ' reseñas)</div>';
                }}
                
                html += '<div style="margin-top: 15px;">';
                html += '<a href="' + product.link + '" target="_blank" rel="noopener noreferrer" class="product-link" onclick="trackProductClick(\'' + product.source + '\', \'' + product.title + '\')">';
                html += '🛒 Comprar en ' + product.source + ' →';
                html += '</a>';
                html += '</div>';
                html += '<div style="margin-top: 10px; font-size: 12px; color: #666;">';
                html += 'ID: ' + (product.product_id || 'N/A') + ' | Tipo: ' + product.part_type;
                html += '</div>';
                html += '</div>';
            }});
            ...
            html += '<div style="background: #f8f9fa; padding: 20px; border-radius: 10px; margin-top: 30px; text-align: center;">';
            html += '<h4 style="color: #1e3c72; margin-bottom: 15px;">💡 Consejos para comprar repuestos</h4>';
            html += '<ul style="text-align: left; color: #666; max-width: 600px; margin: 0 auto;">';
            html += '<li style="margin-bottom: 8px;">✅ Verifica la compatibilidad con tu vehículo antes de comprar</li>';
            html += '<li style="margin-bottom: 8px;">💰 Compara precios entre diferentes tiendas</li>';
            html += '<li style="margin-bottom: 8px;">⭐ Lee las reseñas de otros compradores</li>';
            html += '<li style="margin-bottom: 8px;">🔧 Considera la garantía ofrecida por cada tienda</li>';
            html += '<li style="margin-bottom: 8px;">📞 Contacta al vendedor si tienes dudas sobre compatibilidad</li>';
            html += '</ul>';
            html += '</div>';
...
def search_page():
    ...
    try:
        ...
        function displayPremiumResults(products) {{
            ...
            let html = '<div style="background: linear-gradient(135deg, #e8f5e8 0%, #f0f8f0 100%); padding: 25px; border-radius: 12px; margin: 30px 0;">';
            html += '<h3 style="color: #155724;">✅ ' + products.length + ' repuestos encontrados (Búsqueda Premium)</h3>';
            html += '</div>';
            ...
            products.forEach(product => {{
                ...
                html += '<h4 class="product-title">' + product.title + ' ' + partBadge + '</h4>';
                html += '<div class="product-price">' + product.price + '</div>';
                html += '<div class="product-store"><strong>Tienda:</strong> ' + product.source + '</div>';
                html += '<div style="font-size: 13px; color: ' + availabilityColor + '; font-weight: 600; margin: 8px 0;">📦 ' + availabilityText + '</div>';
                ...
                html += '<button onclick="saveFavorite(\'' + product.title + '\')" style="background: #28a745; color: white; border: none; padding: 5px 10px; border-radius: 4px; cursor: pointer; margin-right: 10px;">❤️ Guardar</button>';
                html += '<a href="' + product.link + '" target="_blank" rel="noopener noreferrer" class="product-link" onclick="trackProductClick(\'' + product.source + '\', \'' + product.title + '\')">🛒 Comprar en ' + product.source + ' →</a>';
                html += '</div>';
                html += '<div style="margin-top: 10px; font-size: 12px; color: #666;">ID: ' + (product.product_id || 'N/A') + ' | Tipo: ' + product.part_type + '</div>';
                html += '</div>';
            }});
        ...
        function updateHistoryDisplay() {{
            ...
            searchHistory.forEach(query => {{
                html += '<span style="display: inline-block; background: #e3f2fd; color: #1976d2; padding: 5px 10px; border-radius: 15px; margin: 5px 5px 5px 0; cursor: pointer;" onclick="document.getElementById(\\'searchQuery\\').value = \\'' + query + '\\'; searchParts();">' + query + '</span>';
            }});
...
if __name__ == '__main__':
    ...
    try:
        ...
        print("   - Dependencias instaladas")# webapp.py - Auto Parts Finder USA - VERSIÓN FINAL LIMPIA
...
def home():
    ...
    try:
        ...
        function displayResults(products, searchInfo) {{
            ...
            let html = '<div style="background: linear-gradient(135deg, #e8f5e8 0%, #f0f8f0 100%); padding: 25px; border-radius: 12px; margin: 30px 0; border-left: 5px solid #28a745;">';
            html += '<h3 style="color: #155724; margin-bottom: 10px;">✅ Encontrados ' + products.length + ' repuestos automotrices</h3>';
            html += '<p style="color: #155724;"><strong>Búsqueda:</strong> ' + (searchInfo.query || 'Imagen');
            if (searchInfo.vehicle) {{
                html += ' | Vehículo: ' + searchInfo.vehicle;
            }}
            html += '</p>';
            html += '<p style="color: #666; font-size: 14px; margin-top: 10px;">Resultados de las mejores tiendas de repuestos en USA</p>';
            html += '</div>';
            html += '<div class="product-grid">';
